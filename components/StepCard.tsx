...
const StepCard: React.FC<Props> = ({ title, icon: Icon, isActive, isCompleted, onClick, onShowPrompt }) => (
  ...
  >
    ...
    <div className="flex-1 min-w-0">
      <h3 className={`font-serif font-bold text-xs sm:text-sm truncate ${isActive ? 'text-stone-900' : 'text-gray-900'}`}>{title}</h3>
      {isCompleted && <span className={`text-[10px] sm:text-xs opacity-70 flex items-center mt-0.5 sm:mt-1 font-sans ${isActive ? 'text-stone-500' : 'text-gray-500'}`}><Check size={12} className="mr-1" /> 已生成</span>}
    </div>
    ...
    {onShowPrompt && (
      <button
        ...
        className={`p-2.5 rounded-md transition-opacity ${isActive ? 'opacity-100 hover:bg-gray-100 text-gray-400 hover:text-black' : 'opacity-0 group-hover:opacity-100 hover:bg-gray-100 text-gray-400 hover:text-black'} ml-1 min-w-[36px] flex items-center justify-center`}
        title="查看提示词"
      >
        <FileText size={16} />
      </button>
    )}
    {isActive && <ChevronRight size={16} className="text-stone-900 ml-2 min-w-[32px] flex items-center justify-center" />}
  </div>
);
