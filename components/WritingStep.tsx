      ...
      try {
        ...
        if (chapterSections.length > viewChapter) {
          ...
          if (titleLineMatch) {
            ...
            if (altTitleMatch) {
              extractedTitle = altTitleMatch[1]
                .trim()
                .replace(/\*\*/g, '')
                .replace(/\*/g, '')
                .replace(/_/g, '')
              ...
              if (keywordTitleMatch) {
                extractedTitle = keywordTitleMatch[1]
                  .trim()
                  .replace(/\*\*/g, '')
                  .replace(/\*/g, '')
                  .replace(/_/g, '')
          ...
          const title = manuallyEditedTitles.has(viewChapter)
            ...
            : extractedTitle;
          console.log(
            `[WritingStep] Final title(after manual edit check): `,
            title,
            `(manuallyEdited: `,
            manuallyEditedTitles.has(viewChapter),
            `)`
          );
          ...
          // 7. 提取本章简述，增强多行内容处理
          const summaryMatch = chapterContent.match(
            /[\*_]*本章简述[\*_]*[:：]\s*([\s\S]*?)(?=\n\s*###|$)/i
          );
          ...
          if (summaryMatch) {
            ...
            const altSummaryMatch = chapterContent.match(
              /[\*_]*(本章简述|本章简介|简述|简介)[\*_]*[:：]\s*([\s\S]*?)(?=\n\s*[\*_]{2,}|$)/i
            );
            if (altSummaryMatch) {
          ...
          console.log(
            `[WritingStep] Final extracted params for chapter ${viewChapter}: `,
            finalParams
          );
          ...
          console.log(
            `[WritingStep] No blueprint section found for chapter ${viewChapter}.Sections available: ${
        ...
        console.error(
          `[WritingStep] Error processing blueprint for chapter ${viewChapter}: `,
          error
        );
...
${THEME_LIBRARY_CONTENT} `;
  ...
  const renderRecBadge = (level: string) => {
    if (level === 'highly_recommended') {
      return (
        <span className="text-[10px] font-bold bg-emerald-50 text-emerald-700 px-1.5 py-0.5 rounded flex items-center border border-emerald-100">
          <ThumbsUp size={10} className="mr-1" /> 强烈推荐
        </span>
      );
    } else if (level === 'recommended') {
      return (
        <span className="text-[10px] font-bold bg-amber-50 text-amber-700 px-1.5 py-0.5 rounded flex items-center border border-amber-100">
          <Star size={10} className="mr-1" /> 推荐
        </span>
      );
    } else if (level === 'not_recommended') {
      return (
        <span className="text-[10px] font-bold bg-gray-100 text-gray-500 px-1.5 py-0.5 rounded flex items-center border border-gray-200">
          <MinusCircle size={10} className="mr-1" /> 一般
        </span>
      );
    }
    return null;
    ...
    <div className="flex flex-col h-full min-h-0 bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
      ...
      <div className="px-4 py-3 border-b border-gray-100 bg-white flex flex-wrap justify-between items-center gap-3 shrink-0">
        <div className="flex items-center space-x-2 sm:space-x-3">
          <button
            ...
            className="p-3 text-gray-400 hover:text-gray-900 rounded-lg hover:bg-gray-50 min-w-[40px] flex items-center justify-center transition-colors lg:hidden"
            title="显示章节列表"
            type="button"
          >
            <FileText size={20} />
      ...
      <div className="flex-1 min-h-0 flex relative">
        ...
        {(isRightPanelOpen || isLeftPanelOpen) && (
          <div
            className="absolute inset-0 bg-black/30 backdrop-blur-sm z-30 lg:hidden"
            onClick={() => {
        ...
        <div
          className={`w-full lg:w-64 bg-gray-50/50 border-r border-gray-100 flex flex-col shrink-0 h-full min-h-0 fixed lg:relative top-0 left-0 z-40 transform transition-transform duration-300 ease-in-out lg:translate-x-0 ${
        ...
        >
          <div className="p-4 border-b border-gray-100 bg-white sticky top-0 z-10 flex justify-between items-center shrink-0">
            <h3 className="text-xs font-bold text-gray-400 uppercase flex items-center tracking-wider">
              <List size={14} className="mr-2 text-gray-900" /> 章节目录
            </h3>
            <button
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar p-2 space-y-1">
            {Array.from({ length: inputs.numberOfChapters || 12 }).map((_, i) => {
              const chapterNum = i + 1;
              const isActive = viewChapter === chapterNum;
              const chapterData = generatedData.chapters[i];
              const chapterTitle = chapterData?.title || '未生成';
              const hasContent = !!chapterData?.content;

              return (
                <button
                  key={chapterNum}
                  onClick={() => {
                    onUpdateViewChapter(chapterNum);
                    setIsLeftPanelOpen(false);
                  }}
                ...
                >
                  <span
                    className={`text-[10px] font-mono font-bold w-12 shrink-0 ${
                  ...
                  <span
                    className={`text-xs truncate flex-1 font-serif ${isActive ? 'font-bold' : ''}`}
                  >
                    {chapterTitle}
                  </span>
                  {hasContent && (
            ...
            <div className="flex flex-col items-center justify-center flex-1 text-gray-900 space-y-6 py-20">
              ...
              <div className="text-center">
                <p className="font-bold text-lg font-serif">
                  {loadingMessage ||
                    `AI 正在${currentChapter?.content ? '重新' : ''}撰写 ${
                      chapterParams.title || `第${viewChapter}章`
                    } `}
                </p>
                {!loadingMessage && (
              ...
              {isThemeGenerating ? (
                ...
                matchedThemes.map((theme) => (
                  <div
                    key={theme.code}
                    onClick={() =>
                      onUpdateSelectedTheme(selectedTheme?.code === theme.code ? null : theme)
                    }
                  ...
                  >
                    <div className="flex justify-between items-start mb-2">
                      <span
                        className={`font-bold ${
                          selectedTheme?.code === theme.code ? 'text-orange-700' : 'text-gray-900'
                        } `}
                      >
                        {theme.name}
                      </span>
                    ...
                    {selectedTheme?.code === theme.code && (
                      <button
                        ...
                        disabled={isGenerating}
                        className={`w-full py-1.5 bg-black hover:bg-gray-800 text-white text-xs font-bold rounded flex items-center justify-center transition-colors shadow-sm ${
                ...
                {!showHumanizeInput ? (
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex space-x-2">
                      ...
                      >
                        {isHumanizeRewriting ? (
                          <RefreshCw className="animate-spin w-3 h-3 mx-auto" />
                        ) : (
                          '确认润色'
                        )}
                      </button>
                ...
                {demonCritique && (
                  <div className="space-y-3 animate-in fade-in slide-in-from-top-2">
                    ...
                    <div className="space-y-2">
                      <button
                        onClick={() => handleApplyDemonRewrite('option1')}
                        disabled={isDemonEditing}
                        className="w-full text-left p-2 hover:bg-gray-50 rounded text-xs text-gray-600 border border-transparent hover:border-gray-200 transition-all truncate"
                      >
                        👉 方案A：强化冲突与悬念
                      </button>
                      <button
                        onClick={() => handleApplyDemonRewrite('option2')}
                        disabled={isDemonEditing}
                        className="w-full text-left p-2 hover:bg-gray-50 rounded text-xs text-gray-600 border border-transparent hover:border-gray-200 transition-all truncate"
                      >
                        👉 方案B：优化节奏与心理
