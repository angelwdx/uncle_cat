...
export default function App() {
  ...
  const [selectedPlotStructure, setSelectedPlotStructure] = useState<string>(
    PLOT_STRUCTURES[0]?.name || '三幕式结构（Three-Act Structure）'
  );
  ...
  useEffect(() => {
    if (isLoaded)
      localStorage.setItem('storymind_instructions', JSON.stringify(stepCustomInstructions));
  ...
  const findLatestStateArchive = (targetChapterNum: number) => {
    ...
    const sortedArchives = [...(generatedData.stateHistory || [])].sort(
    ...
    try {
      ...
      const extractSelectedProposal = (result: string, index: number) => {
        ...
        const proposalRegex = new RegExp(
          `【方案${targetIndex}：.*?】[\s\S]*?(?=【方案${targetIndex + 1}：|$)`,
          'i'
        );
      ...
      const cleanGeneratedResult = (result: string) => {
        ...
        let basicSettings = basicSettingsMatch
          ? `${basicSettingsMatch[1] || '## '}基础设定 (BASIC_SETTINGS)\n${
              basicSettingsMatch[2]
            }`.trim()
          : '';
  ...
  const handleGenerateStep = async (stepId: keyof GeneratedData, customReq: string = '') => {
    ...
    try {
      const template =
        customPrompts[stepDef.promptKey] || PROMPTS[stepDef.promptKey as keyof typeof PROMPTS];
  ...
  const handleGenerateChapter = async (chapterNum: number, params: any, theme: any) => {
    ...
    try {
      ...
      const variables = {
        ...
        selected_theme_info: theme
          ? `已选题材公式：${theme.name} - ${theme.desc}`
          : '未指定特定题材公式，请自行发挥',
  ...
  const handleSyncContext = async (chapterNum: number) => {
    ...
    try {
      ...
      const chapterText =
        currentChapter.content.length > maxChapterLength
      ...
      const variables = {
        ...
        chapter_blueprint: generatedData.blueprint
          ? generatedData.blueprint.split(/###\s*第\s*\d+\s*章/).find((_, i, arr) => {
              ...
              // 这里尝试寻找包含当前章节号附近的内容
              return (
                section.includes(`第${chapterNum}章`) ||
                section.includes(`章名：${currentChapter.title}`)
              );
            }) || '暂无当前章节蓝图'
          : '暂无章节蓝图',
        novel_number: chapterNum,
      ...
      const globalSummaryMatch = result.match(
        /##\s*(?:全局故事摘要|GLOBAL_SUMMARY_UPDATED)[\s\S]*?(?=##\s*(?:角色状态档案|CHARACTER_STATE_UPDATED|当前章节摘要|CURRENT_CHAPTER_SUMMARY)|$)/i
      );
      const charStateMatch = result.match(
        /##\s*(?:角色状态档案|CHARACTER_STATE_UPDATED)[\s\S]*?(?=##\s*(?:当前章节摘要|CURRENT_CHAPTER_SUMMARY|$))/i
      );
      const chapSummaryMatch = result.match(
        /##\s*(?:当前章节摘要|CURRENT_CHAPTER_SUMMARY)[\s\S]*$/i
      );
      ...
      if (globalSummaryContent || charStateContent || chapSummaryContent) {
        setGeneratedData((prev) => {
          ...
          const newArchive: StateArchive = {
            ...
            chapterSummary:
              chapSummaryContent || newData.chapters[chapterNum - 1]?.summary || '暂无',
            timestamp: Date.now(),
          };
  ...
  const getPromptVariables = (promptKey: string, chapterNum?: number) => {
    ...
    if (promptKey === 'CHAPTER_1' || promptKey === 'CHAPTER_NEXT') {
      ...
      if (generatedData.blueprint) {
        const blueprintLines = generatedData.blueprint.split('\n');
        // 查找当前章节，支持更灵活的匹配格式
        const currentChapterRegex = new RegExp(
          `###\\s*第\\s*${currentChapterNum}\\s*章\\s*[:：\\-－\\s]*(.*)`,
          'i'
        );
        // 查找下一章
        ...
        for (const line of blueprintLines) {
          ...
          if (currentMatch) {
            ...
            if (fieldMatch) {
              ...
              if (label.includes('章节定位') || label.includes('本章定位'))
                chapterRole = value.trim();
              if (label.includes('核心作用')) chapterPurpose = value.trim();
              if (label.includes('悬念设置') || label.includes('悬念密度'))
                suspenseLevel = value.trim();
              if (
                label.includes('伏笔埋藏') ||
                label.includes('伏笔操作') ||
                label.includes('埋线取线')
              )
                foreshadowing = value.trim();
              if (
                label.includes('反转指数') ||
                label.includes('认知颠覆') ||
                label.includes('反转程度')
              )
                plotTwistLevel = value.trim();
              if (
                label.includes('本章摘要') ||
                label.includes('本章简述') ||
                label.includes('剧情摘要')
              )
                shortSummary = value.trim();
            }
          } else if (inNextChapter) {
            const fieldMatch = line.match(/^\s*[-*]?\s*\*\*(.*?)[：:]\*\*\s*(.*)/);
      ...
      variables = {
        ...
        selected_theme_info: writingStepState.selectedTheme
          ? `已选题材公式：${writingStepState.selectedTheme.name} - ${writingStepState.selectedTheme.desc}`
          : '未指定特定题材公式，请自行发挥',
        character_state: latestArchive.characterState,
        world_building: generatedData.world || '暂无世界观设定',
        plot_architecture: generatedData.plot || '暂无情节架构',
        ...
        previous_chapter_excerpt:
          previousContent.length > 1500 ? `...${previousContent.slice(-1500)}` : previousContent,
        chapter_summary: latestArchive.chapterSummary || '暂无摘要',
  ...
  const handleGetFullPrompt = (promptKey: string, chapterNum?: number) => {
    let template = '';
    if (promptKey === 'THEME_MATCH_PROMPT')
      template = customPrompts[promptKey] || THEME_MATCH_PROMPT;
    else if (promptKey === 'STATE_UPDATE')
      template = customPrompts[promptKey] || PROMPTS.STATE_UPDATE;
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setInputs((prev) => ({ ...prev, perspective: e.target.value }))
                    }
                  >
                    <option value="">请选择视角...</option>
                    {NARRATIVE_PERSPECTIVES.map((t) => (
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <div className="flex gap-2">
                    <input
                      ...
                      onChange={(e) =>
                        setInputs((prev) => ({ ...prev, novelTitle: e.target.value }))
                      }
                    />
                    <button
                      type="button"
                ...
                <div>
                  ...
                  <textarea
                    ...
                    onChange={(e) =>
                      setInputs((prev) => ({ ...prev, customRequirements: e.target.value }))
                    }
                  />
                </div>
              </div>
      ...
      return (
        <WritingStep
          ...
          onUpdateViewChapter={(chapterNum) =>
            setWritingStepState((prev) => ({ ...prev, viewChapter: chapterNum }))
          }
          onUpdateSelectedTheme={(theme) =>
            setWritingStepState((prev) => ({ ...prev, selectedTheme: theme }))
          }
          ...
          <div className="flex flex-wrap items-center justify-between bg-white px-4 py-3 rounded-xl border border-gray-100 shadow-sm gap-3">
            ...
            <div className="flex items-center space-x-2 sm:space-x-3 w-auto justify-end">
              ...
              <button
                ...
                className={`flex items-center px-4 py-1.5 bg-white hover:bg-gray-50 text-stone-900 border border-gray-200 hover:border-gray-400 font-serif font-medium rounded-lg transition-all ${
                  isGenerating ? 'opacity-50 cursor-not-allowed' : 'shadow-sm hover:shadow'
                } min-h-[36px] justify-center text-sm whitespace-nowrap`}
              >
                {isGenerating ? (
    ...
    <div className="h-screen w-screen bg-stone-950 text-stone-100 flex font-sans">
      ...
      >
        <div className="p-8 border-b border-gray-100 flex flex-col items-center">
          ...
          />
          <h1 className="font-serif text-xl font-bold tracking-wider text-gray-900">
            Story Mind <span className="text-gray-400 font-light text-sm align-top">1.0</span>
          </h1>
            ...
            <StepCard
              ...
              isCompleted={
                idx === 0
                  ? isInitCompleted
                  : step.id === 'writing'
                  ? generatedData.chapters.length > 0
                  : idx > 0 && idx < 7
            ...
            <div className="flex items-center gap-2 flex-1 min-w-0">
              <div
                className={`w-1.5 h-1.5 rounded-full shrink-0 ${
                  apiConfig.apiKey ? 'bg-black' : 'bg-gray-300'
                }`}
              />
